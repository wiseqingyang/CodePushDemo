...
1. 一个可被 `import/require` 的 js 模块，能在 app 运行中访问服务的模块（例如 检查更新，检查当前应用更新的元数据）。

2. 能让 React Native app 自己选择正确 js 包位置的原生 API。

The following sections describe the shape and behavior of these APIs in detail:

### JavaScript API Reference

引用 `react-native-code-push` 时, 除了[组件装饰器](#codepush)外还提供以下顶层方法:

* [allowRestart](#codepushallowrestart): 恢复更新安装后预制好的重启机制， 另外，当有更新但重启被禁止的时候立即重启 app。这是一个高级 API，仅在使用 `disallowRestart` 明确禁止重启的时候使用。

* [checkForUpdate](#codepushcheckforupdate): 向服务器请求当前部署是否有更新。

* [disallowRestart](#codepushdisallowrestart): 暂停更新安装后重启。这个是高级 API，能够很好的保证对用户不会有任何打断。

* [getCurrentPackage](#codepushgetcurrentpackage): 检索当前安装的更新的元数据（例如 描述，安装时间，大小）。 *注意: 从 `v1.10.3-beta` 开始, 不在推荐此方法，推荐 [`getUpdateMetadata`](#codepushgetupdatemetadata)*。

* [getUpdateMetadata](#codepushgetupdatemetadata): 检索当前安装的更新的元数据（例如 描述，安装时间，大小）。

* [notifyAppReady](#codepushnotifyappready):通知 CodePush runtime 把已经安装的更新视为成功。如果是手动检查更新和手动安装更新（例如：完全没有使用 [sync](#codepushsync)处理更新），那么**必须**要调用本方法，否则 CodePush 会认为本次更新失败，下载启动 app 的时候自动回滚到上一个版本。

* [restartApp](#codepushrestartapp): 立即重启 app。 如果有更新挂起，会立即展现更新内容给用户。如果没有，此方法只会简单的杀掉进程重启。

* [sync](#codepushsync): 只需调用一次就能完成检查、下载、安装更新。除非需要自定义 UI 或者自定义操作，我们建议大多数开发者集成 CodePush 的时候使用本方法。

* [clearUpdates](#clearupdates): 清除所有已下载的 CodePush 更新，当切换到更老版本的部署环境的时候很有用。 
   
    _注意: 除此之外（其他情况下 CodePush 会自动调用本方法）不建议使用此方法，否则会导致不可预料结果。_
...
被用来在能够同步所挂载的 js 文件和资源的更高级的 React 组件中包装 Rect 组件。高阶组件内部生命周期函数 `componentDidMount` 中调用触发检查更新、下载更新、安装更新的 [`sync`](#codepushsync) 函数。
...
* __checkFrequency__ *(codePush.CheckFrequency)* - 标明触发频率。默认是 `codePush.CheckFrequency.ON_APP_START`。具体查看枚举 [`CheckFrequency`](#checkfrequency) 获取更多选项的描述及作用。
